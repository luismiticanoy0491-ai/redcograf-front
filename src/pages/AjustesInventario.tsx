...
function AjustesInventario() {
  ...
  const fetchProductos = () => {
    setLoading(true);
    API.get("/productos?tipo=producto")
      .then(res => {
    ...
    <div className="max-w-[1400px] mx-auto animate-in fade-in duration-700 pb-20">
      ...
      <div className="relative group mb-10">
        ...
        <input 
          ...
          className="w-full pl-16 pr-8 py-5 bg-white border border-slate-200 rounded-[32px] font-medium text-slate-700 outline-none focus:ring-4 focus:ring-indigo-50 focus:border-indigo-400 transition-all shadow-sm"
        />
      </div>
      ...
      <div className="bg-white rounded-[48px] border border-slate-200 shadow-sm overflow-hidden">
        <div className="p-8 border-b border-slate-100 flex items-center justify-between">
            <h3 className="text-lg font-medium text-slate-900 uppercase tracking-widest flex items-center gap-2">
                <span className="w-2 h-6 bg-indigo-500 rounded-full"></span> Panel de Nivelación
            </h3>
            <span className="text-[10px] font-medium text-slate-400 uppercase tracking-widest bg-slate-50 px-4 py-1 rounded-full">{filtered.length} Items en Pantalla</span>
        </div>

        <div className="overflow-x-auto">
            <table className="w-full text-left">
                <thead>
                    <tr className="text-[10px] font-medium text-slate-400 uppercase tracking-widest border-b border-slate-100 bg-white">
                ...
                <tbody className="divide-y divide-slate-50">
                    {loading && productos.length === 0 ? (
                        <tr><td colSpan={5} className="py-24 text-center text-slate-400 font-medium uppercase text-[10px] tracking-widest">Sincronizando almacén...</td></tr>
                    ) : filtered.length === 0 ? (
                        <tr><td colSpan={5} className="py-24 text-center text-slate-300 font-medium italic opacity-50">No se encontraron productos para ajustar.</td></tr>
                    ) : (
                        filtered.map((p: Producto) => {
                            ...
                            return (
                                <tr key={p.id} className="group hover:bg-slate-50/50 transition-colors">
                                    <td className="px-8 py-6 font-medium text-slate-400 text-xs">{(p.referencia || "SIN-SKU")}</td>
                                    <td className="px-8 py-6">
                                        <div className="font-medium text-slate-900 uppercase leading-tight group-hover:text-indigo-600 transition-colors">{p.nombre}</div>
                                        <div className="text-[10px] font-medium text-slate-400 mt-1 uppercase tracking-tighter italic">{p.categoria}</div>
                                    </td>
                                    <td className="px-8 py-6 text-center">
                                        <span className={`px-4 py-1 rounded-full text-[10px] font-medium uppercase tracking-widest ${
                                    ...
                                    <td className="px-8 py-6">
                                        <div className="flex items-center justify-center gap-3">
                                            <button 
                                                ...
                                                className="w-10 h-10 rounded-2xl bg-white border border-slate-200 text-slate-400 font-medium hover:bg-rose-50 hover:text-rose-500 hover:border-rose-100 transition-all flex items-center justify-center shadow-sm"
                                            >-</button>
                                            <input 
                                                type="text" 
                                                value={valorLocal !== undefined ? valorLocal : ''}
                                                onChange={(e) => handleCantChange(p.id as number, e.target.value)}
                                                className={`w-20 text-center font-medium py-2 rounded-2xl border outline-none focus:ring-4 transition-all ${
                                            ...
                                            <button 
                                                onClick={() => handleSum(p.id as number, 1)}
                                                className="w-10 h-10 rounded-2xl bg-white border border-slate-200 text-slate-400 font-medium hover:bg-emerald-50 hover:text-emerald-500 hover:border-emerald-100 transition-all flex items-center justify-center shadow-sm"
                                            >+</button>
                                        </div>
                                    </td>
                                    <td className="px-8 py-6 text-center">
                                        <button 
                                            ...
                                            className={`px-6 py-3 rounded-2xl text-[10px] font-medium uppercase tracking-widest transition-all ${
